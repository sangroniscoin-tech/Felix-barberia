...
// Datos del responsable del tratamiento, para el aviso de privacidad. La ley
// obliga a un autónomo a publicarlos, así que son públicos a propósito: no son
// una credencial y no van en variables de entorno. La razón social es su propio
// nombre porque Félix es autónomo.
const RESPONSABLE_NOMBRE = "Félix Daniel Sangronis Castillo";
const RESPONSABLE_NIF = "61564701A";
...
// Las fotos de los servicios. Salen de `public/`, que Vite publica en la raíz
// del sitio, así que las sirve esta misma web.
//
// **Ninguna puede volver a ser una URL de fuera.** Venían de Unsplash y eso
// entregaba la IP de cada cliente a una empresa ajena en cada visita, con el
// aviso de privacidad diciendo por escrito que aquí no interviene nadie más
// que Supabase y Vercel (#111, #113, #119). Es la misma trampa que las
// tipografías de Google: se ve bien, funciona, y rompe la promesa en silencio.
//
// Ficheros y no `data:` en base64: el paquete de JavaScript ya pasa de 500 kB,
// y así el navegador las cachea por separado en vez de rebajarlas con cada
// despliegue.
const FOTOS_FIJAS = {
  corte: "/servicios/corte.jpg",
  barba: "/servicios/barba.jpg",
  corte_barba: "/servicios/corte-barba.jpg",
};

// El resto del juego, para los servicios que Félix añade desde el panel. Van
// en orden: el primer servicio sin foto fija coge la primera, y así.
//
// Son doce para dos que hoy las necesitan. La holgura es a propósito: cuando
// se acaben, alguien tiene que añadir ficheros aquí, y es mejor que eso pase
// dentro de años que a la tercera vez que Félix inventa un servicio.
const FOTOS_LIBRES = [
  "/servicios/diseno.jpg",
  "/servicios/corte-maquina.jpg",
  "/servicios/barba-navaja.jpg",
  "/servicios/barba-tijera.jpg",
  "/servicios/corte-tijera.jpg",
  "/servicios/nuca.jpg",
  "/servicios/herramientas.jpg",
  "/servicios/maquinas.jpg",
  "/servicios/sillon.jpg",
  "/servicios/sala.jpg",
  "/servicios/local.jpg",
  "/servicios/sillones.jpg",
];

// Reparte una foto a cada servicio, y NINGUNA se da dos veces.
//
// Esto recibe la lista entera a propósito. Lo que había antes miraba un
// servicio de uno en uno y elegía por un hash de su identificador: una función
// que no ve a los demás no puede saber qué fotos están cogidas, así que dos
// servicios caían en la misma y eso es exactamente lo que Félix reportó —
// "Corte + diseño" y "Corte jubilado" con la misma foto. Un hash sobre un
// juego finito siempre puede repetir; repartir sin reemplazo no puede.
//
// El orden es el mismo en el que se pintan, que viene del `sort_order` del
// servidor. Añadir un servicio al final no le cambia la foto a ninguno de los
// que ya estaban.
//
// Devuelve un Map de id -> ruta, y **null** para el servicio que se quede sin
// foto si algún día hay más servicios que fotos. Null significa "píntalo sin
// imagen", no "repite una": una tarjeta con el icono de tijeras se arregla
// mandando una foto más, una tarjeta duplicada es el fallo que ya se reportó.
function repartirFotos(services) {
  const reparto = new Map();
  const usadas = new Set();

  // Primero las fijas, para que su foto quede apartada antes de repartir el
  // resto. Al revés, un servicio nuevo podría llevarse la foto de `corte`.
  for (const s of services) {
    const fija = FOTOS_FIJAS[s.id];
    if (fija) {
      reparto.set(s.id, fija);
      usadas.add(fija);
    }
  }
  ...
  let siguiente = 0;
  for (const s of services) {
    if (reparto.has(s.id)) continue;
    while (siguiente < FOTOS_LIBRES.length && usadas.has(FOTOS_LIBRES[siguiente])) siguiente++;
    const libre = siguiente < FOTOS_LIBRES.length ? FOTOS_LIBRES[siguiente] : null;
    if (libre) {
      usadas.add(libre);
      siguiente++;
    }
    reparto.set(s.id, libre);
  }
...
export default function FelixBarberiaApp() {
  ...
  return (
    <div style={{ fontFamily: "Inter, sans-serif", background: "#0B0B0A", minHeight: "100vh", color: BONE }}>
      {/* Las letras se sirven desde esta misma web, y no desde Google (#111).
          Aquí había un @import al CSS de tipografías de Google, así que el
          navegador de cada visitante se las pedía a él en cada visita y le
          entregaba su IP de paso. Eran las únicas peticiones automáticas a un
          tercero que hacía esta web.

          Las reglas de abajo son las MISMAS que servía Google —familias,
          grosores, unicode-range y font-display: swap— con la URL cambiada.
          Se copian tal cual a propósito: cualquier "simplificación" ahí cambia
          cómo se ve la web, y la condición de este cambio era que se viera
          exactamente igual.

          Los ficheros son variables, y Google servía el mismo para los cuatro
          grosores de Inter: por eso en public/fonts/ hay 6 y no 14, y varias
          reglas apuntan al mismo fichero. Solo latino y latino extendido; el
          cirílico, el griego y el vietnamita que Google añadía no los usa nadie
          aquí, y eran ficheros que el cliente nunca iba a abrir.

          Este comentario vive FUERA de la plantilla del <style> a propósito: lo
          que se escriba dentro viaja al navegador de cada cliente, y no tiene
          ningún sentido mandarle a él la explicación —ni, precisamente aquí, el
          nombre del dominio que este cambio quita—.

          La licencia OFL de las dos, que es lo que permite alojarlas, está en
          public/fonts/. */}
      <style>{`
        @font-face {
          font-family: 'Inter';
      ...
      <TopChrome menuOpen={menuOpen} setMenuOpen={setMenuOpen} onAdmin={() => goTo("admin")} onInicio={() => goTo("inicio")} onMisCitas={() => goTo("misCitas")} onContacto={() => goTo("contacto")} onGaleria={() => goTo("galeria")} onPrivacidad={() => goTo("privacidad")} />
      ...
      <div className="wood-bg" style={{ paddingBottom: 78, minHeight: "100vh" }}>
        ...
        {/* Texto fijo y legal: no depende de `loaded` ni de `/api/bootstrap`,
            así que se lee igual aunque la agenda no cargue. */}
...
function TopChrome({ menuOpen, setMenuOpen, onAdmin, onInicio, onMisCitas, onContacto, onGaleria, onPrivacidad }) {
  return (
      ...
      {menuOpen && (
        <div onClick={() => setMenuOpen(false)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 29 }}>
          <div onClick={(e) => e.stopPropagation()} className="fade-in" style={{ background: "#161513", borderBottom: `1px solid rgba(201,162,39,0.3)`, padding: "70px 20px 20px", display: "flex", flexDirection: "column", gap: 4 }}>
            {[["Inicio", onInicio], ["Mis citas", onMisCitas], ["Galería", onGaleria], ["Contacto", onContacto], ["Privacidad", onPrivacidad], ["Panel de barbero", onAdmin]].map(([label, fn]) => (
...
function Inicio({ services, onReservar, schedule, onContacto, loaded, loadFailed }) {
  ...
  // El reparto se hace una vez sobre la lista entera, no servicio a servicio:
  // es lo único que garantiza que no se repita ninguna foto.
  const fotos = useMemo(() => repartirFotos(services), [services]);
  return (
    <div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: 12 }}>
          {services.map((s) => (
            <button key={s.id} onClick={() => onReservar(s.id)} className="card fade-in" style={{ textAlign: "left", borderRadius: 14, overflow: "hidden", cursor: "pointer", display: "flex", flexDirection: "column" }}>
              {/* Sin foto libre, el hueco se queda con el fondo de la casa y el
                  icono que ya va encima. Repetir una foto de otro servicio es
                  justamente lo que no puede pasar. */}
              <div style={{ position: "relative", height: 110, background: "#141412" }}>
                {fotos.get(s.id) ? (
                  <img src={fotos.get(s.id)} alt={s.name} style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} />
                ) : (
                  <div style={{ width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center" }}>
                    <Scissors size={30} color="rgba(201,162,39,0.35)" />
                  </div>
                )}
...
// El aviso de privacidad que la ley obliga a tener porque la web guarda el
// nombre, el teléfono y el correo de quien reserva. Es contenido fijo: no llama
// a ninguna API y no mira `loaded`, para que se pueda leer aunque la agenda no
// cargue. Todo lo que dice tiene que ser verdad — no hay encargados aparte de
// los tres que se nombran, no hay transferencias fuera de la UE, no hay
// publicidad ni perfilado. Un aviso que miente es peor que no tenerlo.
function Privacidad({ embedded = false }) {
  return (
    <div style={{ maxWidth: 480, margin: "0 auto", padding: embedded ? "0 20px 24px" : "78px 20px 40px" }}>
      <h2 className="display" style={{ fontSize: 22, fontWeight: 700, marginBottom: 4 }}>Aviso de privacidad</h2>
      <div style={{ width: 40, height: 2, background: GOLD, marginBottom: 20 }} />

      <p style={{ fontSize: 14, lineHeight: 1.65, color: "#c9c3b6", margin: "0 0 20px" }}>
        Aquí te contamos, en palabras claras, qué hacemos con los datos que nos das cuando
        reservas una cita.
      </p>

      <PrivacidadBloque titulo="Quién trata tus datos">
        <p style={{ margin: "0 0 8px" }}>
          El responsable es <strong style={{ color: BONE }}>{RESPONSABLE_NOMBRE}</strong>, que trabaja
          como autónomo bajo su propio nombre (Félix Barbería).
        </p>
        <ul style={{ margin: 0, paddingLeft: 18 }}>
          <li>NIF: <strong style={{ color: BONE }}>{RESPONSABLE_NIF}</strong></li>
          <li>Domicilio: <strong style={{ color: BONE }}>{RESPONSABLE_DOMICILIO}</strong></li>
          {RESPONSABLE_EMAIL && (
            <li>
              Correo: <a href={`mailto:${RESPONSABLE_EMAIL}`} style={{ color: GOLD }}>{RESPONSABLE_EMAIL}</a>
            </li>
          )}
          <li>
            WhatsApp:{" "}
            <a href={`https://wa.me/${BARBER_WHATSAPP}`} target="_blank" rel="noopener noreferrer" style={{ color: GOLD }}>
              +34 610 97 57 33
            </a>
          </li>
        </ul>
      ...
      <PrivacidadBloque titulo="Qué datos recogemos">
        <p style={{ margin: "0 0 8px" }}>Solo lo que hace falta para tener la cita en pie:</p>
        <ul style={{ margin: 0, paddingLeft: 18 }}>
          <li>Tu <strong style={{ color: BONE }}>nombre</strong>.</li>
          <li>Tu <strong style={{ color: BONE }}>teléfono</strong>.</li>
          <li>Tu <strong style={{ color: BONE }}>correo electrónico</strong>, solo si lo escribes: es opcional.</li>
          <li>El servicio, el día y la hora que eliges.</li>
        </ul>
        <p style={{ margin: "8px 0 0" }}>
          Nada más. No te pedimos DNI, ni dirección, ni datos de pago, porque en la barbería se
          paga allí mismo.
        </p>
      </PrivacidadBloque>

      <PrivacidadBloque titulo="Para qué los usamos">
        <ul style={{ margin: 0, paddingLeft: 18 }}>
          <li>Para gestionar tu reserva y saber quién viene y a qué hora.</li>
          <li>Para poder avisarte si hay un cambio o si hay que cancelar la cita.</li>
          <li>Para que tú mismo puedas consultar y cancelar tu cita desde «Mis citas», buscándola con tu teléfono.</li>
        </ul>
      </PrivacidadBloque>

      <PrivacidadBloque titulo="Con qué derecho los guardamos">
        <p style={{ margin: 0 }}>
          Porque es lo que hace falta para cumplir la reserva que tú mismo pides. No te pedimos
          que aceptes nada aparte de eso, y por eso no hay ninguna casilla que marcar: si
          reservas, necesitamos tu nombre y tu teléfono para atenderte.
        </p>
      </PrivacidadBloque>
      ...
      <PrivacidadBloque titulo="Quién más los ve">
        <p style={{ margin: "0 0 8px" }}>
          Además de la barbería, tus datos pasan por las empresas que hacen funcionar esta web.
          No hay ninguna más:
        </p>
        <ul style={{ margin: 0, paddingLeft: 18 }}>
      ...
      <PrivacidadBloque titulo="Cookies">
        <p style={{ margin: 0 }}>
          Esta web no usa cookies de publicidad ni de estadísticas, y no te sigue por internet.
          Lo único que se guarda en tu navegador es la sesión del panel del barbero, que es
          estrictamente necesaria para que Félix pueda entrar en él.
        </p>
      </PrivacidadBloque>

      <PrivacidadBloque titulo="Tus derechos">
        <p style={{ margin: "0 0 8px" }}>Puedes pedirnos en cualquier momento:</p>
        <ul style={{ margin: 0, paddingLeft: 18 }}>
          <li><strong style={{ color: BONE }}>Acceso</strong>: saber qué datos tuyos tenemos.</li>
          <li><strong style={{ color: BONE }}>Rectificación</strong>: corregir lo que esté mal.</li>
          <li><strong style={{ color: BONE }}>Supresión</strong>: que los borremos.</li>
          <li><strong style={{ color: BONE }}>Oposición</strong>: que dejemos de usarlos.</li>
          <li><strong style={{ color: BONE }}>Limitación</strong>: que los guardemos pero no los usemos.</li>
          <li><strong style={{ color: BONE }}>Portabilidad</strong>: que te demos una copia.</li>
        </ul>
        <p style={{ margin: "10px 0 0" }}>
          Para pedir cualquiera de estas cosas,{RESPONSABLE_EMAIL ? <>{" "}escríbenos a{" "}
          <a href={`mailto:${RESPONSABLE_EMAIL}`} style={{ color: GOLD }}>{RESPONSABLE_EMAIL}</a>. También puedes hacerlo por WhatsApp al{" "}</> : <>{" "}escríbenos por WhatsApp al{" "}</>}
          <a href={`https://wa.me/${BARBER_WHATSAPP}`} target="_blank" rel="noopener noreferrer" style={{ color: GOLD }}>
            +34 610 97 57 33
          </a>{" "}
          o pasándote por la barbería, en {RESPONSABLE_DOMICILIO}. Dinos tu nombre y el teléfono
          con el que reservaste, para poder encontrar tus citas.
        </p>
      </PrivacidadBloque>

      <PrivacidadBloque titulo="Si no te hacemos caso">
        <p style={{ margin: 0 }}>
          Si crees que no tratamos bien tus datos o que no atendemos lo que pides, puedes
          reclamar ante la <strong style={{ color: BONE }}>Agencia Española de Protección de Datos (AEPD)</strong>,
          en{" "}
          <a href="https://www.aepd.es" target="_blank" rel="noopener noreferrer" style={{ color: GOLD }}>www.aepd.es</a>.
        </p>
      </PrivacidadBloque>

      <p style={{ fontSize: 12, color: SMOKE, textAlign: "center", margin: "18px 0 0" }}>
...
// El mismo aviso, abierto encima de la reserva. Se cierra por el botón o
// tocando fuera; nada de lo escrito en el formulario se pierde.
function PrivacidadOverlay({ onClose }) {
  useEffect(() => {
  ...
  return (
    <div
      onClick={onClose}
      style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", zIndex: 40, display: "flex", alignItems: "flex-end", justifyContent: "center" }}
    >
      <div
        role="dialog"
        aria-modal="true"
        ...
        className="fade-in"
        style={{ background: "#0B0B0A", borderTop: `1px solid rgba(201,162,39,0.35)`, borderRadius: "16px 16px 0 0", width: "100%", maxWidth: 520, maxHeight: "88vh", overflowY: "auto", WebkitOverflowScrolling: "touch" }}
      >
        <div style={{ position: "sticky", top: 0, background: "#0B0B0A", display: "flex", justifyContent: "flex-end", padding: "10px 12px 4px", zIndex: 1 }}>
          <button onClick={onClose} aria-label="Cerrar" style={{ background: "none", border: "1px solid rgba(255,255,255,0.25)", borderRadius: 999, width: 34, height: 34, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: BONE }}>
            <X size={16} />
          </button>
        </div>
        <Privacidad embedded />
...
function PrivacidadBloque({ titulo, children }) {
  return (
    <section className="card" style={{ borderRadius: 14, padding: 18, marginBottom: 12 }}>
      <h3 style={{ fontSize: 15, fontWeight: 700, color: "#f5f1e8", margin: "0 0 10px" }}>{titulo}</h3>
      <div style={{ fontSize: 13.5, lineHeight: 1.65, color: "#c9c3b6", wordBreak: "break-word" }}>{children}</div>
    </section>
  );
}
    ...
    <div style={{ maxWidth: 480, margin: "0 auto", padding: "78px 18px 40px" }}>
      ...
      {step === dataStepNum && (
        <div className="fade-in">
          ...
          {/* El aviso va aquí, justo donde se entregan los datos. Es un enlace,
              no una casilla: la base jurídica es la propia reserva, y una
              casilla obligatoria de más solo estorbaría. */}
          <p style={{ fontSize: 11.5, lineHeight: 1.55, color: SMOKE, margin: "14px 0 0" }}>
            Al reservar aceptas cómo tratamos tus datos.{" "}
            <button
              type="button"
              onClick={() => setVerPrivacidad(true)}
              style={{ background: "none", border: "none", padding: 0, color: GOLD, fontSize: 11.5, fontFamily: "inherit", textDecoration: "underline", cursor: "pointer" }}
            >
              Leer el aviso de privacidad
            </button>
          </p>
          {/* Se abre encima, no cambiando de pantalla: irse a otra vista
              desmontaría la reserva a medias y perdería la hora guardada y lo
              ya escrito. Es el mismo texto que la página del menú. */}
          {verPrivacidad && <PrivacidadOverlay onClose={() => setVerPrivacidad(false)} />}
          <button disabled={!allNamesFilled || !isPhoneValid(phone) || !isEmailValid(email) || submitting} onClick={confirmBooking} className="gold-btn" style={{ width: "100%", marginTop: 14, padding: 14, borderRadius: 12, fontWeight: 700, fontSize: 14 }}>
...
function AdminPanel({ services, setServices, barbers, setBarbers, appointments, closes, saveClose, removeClose, holds, createAppointment, cancelAppointment, setAppointmentNoShow, setAppointmentCharged, refreshAppointments, blockedDays, setBlockedDays, festivos, setFestivos, vacationRanges, setVacationRanges, blockedRanges, setBlockedRanges, portfolio, setPortfolio, waitlist, markWaitlistNotified, removeWaitlistEntry, schedule, setSchedule, loaded, avisoDestino, loadCancelledFor, lastBackup, descargarCopia, onAvisoAplicado }) {
  ...
  // Dinero cobrado, repartido por cómo se pagó. Vive debajo de `hasPassed` y
  // de `now` porque los usa: subirlo lo rompería antes de pintar nada.
  //
  // Quien no se presentó no paga, así que no suma. `apptPrice` se queda
  // valorando la cita igual —lo que cambia es quién entra en la suma, no
  // cuánto vale— porque lo perdido por ausencias se calcula con la misma
  // función y necesita poder valorar precisamente las que no se cobraron.
  //
  // Sólo entra lo que YA HA PASADO. Una cita reservada para la semana que
  // viene no es dinero cobrado, es dinero previsto; contarla llenaría "sin
  // marcar" —lo que a Félix se le ha olvidado apuntar— de citas que
  // sencillamente todavía no han llegado.
  //
  // Las canceladas no aparecen por aquí: `admin-data` no se las sirve al panel.
  //
  // El recuento de citas sí cuenta a los ausentes, a propósito: el hueco lo
  // ocuparon igual y no se le pudo dar a otro. La diferencia entre lo
  // reservado y lo cobrado es justo lo que Félix quiere ver.
  //
  ...
  function marksBreakdown(list) {
    ...
    for (const a of list) {
      ...
      const p = apptPrice(a);
      out.total += p;
      // Cualquier cosa que no sea uno de los tres cae en "sin marcar", que es
      // donde tiene que caer un null y donde no hace daño nada inesperado.
      if (a.paymentMethod === "efectivo") out.efectivo += p;
      else if (a.paymentMethod === "tarjeta") out.tarjeta += p;
      else if (a.paymentMethod === "bizum") out.bizum += p;
      else out.sinMarcar += p;
    }
    return out;
  }
...
// dos cifras viven en la misma pantalla. `tone="apagada"` la pinta en gris,
// para el dinero que sí entró pero del que no consta cómo.
//
